...
mod tests {
    ...
    /// put the word "source" on the wire.
    ...
    #[test]
    fn import_declaration_serializes_source_phase() {
        let phased = parse_and_serialize_with_filename("import source x from \"m\";", "test.js");
        assert!(
            string_buffer_contains(&phased, "source"),
            "`import source x from \"m\"` should serialize phase=\"source\""
        );

        let unphased = parse_and_serialize_with_filename("import x from \"m\";", "test.js");
        assert!(
            !string_buffer_contains(&unphased, "source"),
    ...
    #[test]
    fn import_expression_serializes_source_phase() {
        let phased = parse_and_serialize_with_filename("import.source(\"m\");", "test.js");
        assert!(
            string_buffer_contains(&phased, "source"),
            "`import.source(\"m\")` should serialize phase=\"source\""
        );
