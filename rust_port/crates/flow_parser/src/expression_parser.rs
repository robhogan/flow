...
fn import_expr(env: &mut ParserEnv) -> Result<expression::Expression<Loc, Loc>, Rollback> {
    let (loc, mut inner) = with_loc(None, env, |env| {
        ...
        if eat::maybe(env, TokenKind::TPeriod)? {
            // `import.source(specifier)` — the source phase of a dynamic
            // import. Distinguished from `import.meta` by the property name;
            // `import.defer(...)`, when it lands, is the same shape.
            if matches!(peek::token(env), TokenKind::TIdentifier { raw, .. } if raw == "source") {
                eat::token(env)?;
                return import_call(env, leading, Some(statement::ImportPhase::Source));
            }
            ...
            // identifier `meta`"; the call itself parses as unphased.
