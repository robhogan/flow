...
pub mod statement {
    ...
    /// const x = await import.source("m");
    /// ```
    ///
    /// [None] on an unmodified `import` / `import()`.
    ///
    /// The deferred evaluation phase (`import defer * as ns from "m"`,
    /// `import.defer("m")`) is not parsed yet. Adding it is a `Defer` variant
    /// here plus the parse and validation arms the compiler will point at:
    /// nothing about the field, the ESTree `phase` property or the WASM wire
    /// format needs to change to carry it.
    #[derive(
        Debug,
        Clone,
        Copy,
...
pub mod expression {
    ...
    pub struct Import<M: Dupe, T: Dupe> {
        ...
        /// [Some] for a phase-modified dynamic import (`import.source(...)`),
